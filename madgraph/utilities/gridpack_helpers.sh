set_run_card_pdf () {
    ...
    if grep -q -e "\$DEFAULT_PDF_SETS" $CARDSDIR/${name}_run_card.dat; then
        local central_set=$(python ${script_dir}/python/getMG5_aMC_PDFInputs.py -f "central" -c 2017 $pdfExtraArgs)
        ...
        local central_set=$(python ${script_dir}/python/getMG5_aMC_PDFInputs.py -f "central" -c 2016 $pdfExtraArgs)
